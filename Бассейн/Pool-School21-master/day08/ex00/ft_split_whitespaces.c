...
/*   Updated: 2019/08/10 13:45:43 by snorcros         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		*characters_in_word(char *str)
{
	int		i;
	int		*counts;
	int		j;
	...
	i = 0;
	count = 0;
	counts = (int*)malloc(sizeof(int) * count_words(str));
	while (str[i] != '\0')
	{
		while (str[i] == ' ' || str[i] == '\t' || str[i] == '\n')
			i++;
...
char	**ft_split_whitespaces(char *str)
{
	...
	if (count_str != 0)
	{
		characters_word = characters_in_word(str);
		if (ans == NULL)
			return (0);
		while (i < count_str)
		{
